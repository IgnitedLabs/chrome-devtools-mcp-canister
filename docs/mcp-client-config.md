...
1. Start the shared Chrome daemon once (auto-restarts with Docker):
   ...
   docker compose up -d
   ```
   This runs `chrome-devtools-mcp-canister-chrome` — a bare headless Chrome
   with CDP on port 9222, **not** the MCP server itself. See the comment
   block at the top of `docker-compose.yaml` for why.
